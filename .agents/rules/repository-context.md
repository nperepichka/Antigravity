...
This workspace is the local staging and working copy for the user's global Google Antigravity configuration — encompassing global rules (`GEMINI.md` / `AGENTS.md`), 8 lifecycle engineering workflows (`config/global_workflows/`), 31 specialized domain skills (`config/skills/`), and ecosystem templates (`config/templates/`). All files outside `.agents/` mirror global configuration assets.
...
- **Module Structure:** `GEMINI.md` / `AGENTS.md` (core rules & guardrails) → `config/global_workflows/` (context, investigate, explain, implement, debug, review, describe, checkpoint) → `config/skills/` (domain capability bundles) → `config/templates/` (lifecycle hooks & configs).

## Non-Obvious Conventions & Guardrails
- **Tandem Lifecycle Synergy:** All 8 workflows form a unified pipeline; changes to one workflow must preserve interoperability invariants with the others.
- **Read-Only Discovery Loops:** `/context`, `/investigate`, `/explain`, and `/review` workflows are strictly read-only — zero code modifications or package operations.
