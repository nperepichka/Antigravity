...
This workspace is the local staging and working copy for the user's global Google Antigravity configuration — encompassing global rules (`GEMINI.md`), 7 lifecycle engineering workflows (`config/global_workflows/`), and 31 specialized domain skills (`config/skills/`). All files outside `.agents/` mirror global configuration assets.
