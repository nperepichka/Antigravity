...
- Генерує майстер-план `.local/tasks/**/00_overview.md` та фазові специфікації `01_<name>.md`.

### 2. `/implement` (Автономна розробка та перевірка)
**Файл:** `config/global_workflows/implement.md`
- Автоматично підхоплює фази від `/investigate` (*Fast-Track*) без повторного планування.
- Двофазний цикл: **Phase I** (компіляція, швидкі точкові тести, візуальна верифікація) + **Phase II** (стратегічний аудит SOLID та чистоти).
